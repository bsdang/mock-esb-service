...
:)
declare function json:iv($value as item()) as xs:string {
  if( $value instance of xs:integer or $value instance of xs:double or 
            $value instance of xs:decimal or $value instance of xs:float) then 
            xs:string($value)
        else
            fn:concat('"',json:escapeJSONString(xs:string($value)),'"')
...
) as xs:string
{
    ...
    let $string := replace($string, codepoints-to-string(60), '')
    let $string := replace($string, codepoints-to-string(62), '')
    return $string
};